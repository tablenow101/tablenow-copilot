...
export function loadRuntimeConfig(environment: NodeJS.ProcessEnv = process.env): RuntimeConfig {
  ...
  if ((config.APP_ENV === "production" || config.APP_ENV === "preview" || environment.VERCEL === "1") && config.AUTH_FIXED_OTP) {
    throw new Error("AUTH_FIXED_OTP is forbidden in deployed environments");
  }
