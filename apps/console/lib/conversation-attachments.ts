...
export type ConversationAttachment = { id: string; name: string; byteSize: number };
...
export async function requestAttachment<T>(method: "GET" | "POST" | "DELETE", id?: string, body?: string): Promise<T> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 20_000);
  try {
    return await api<T>(`/v1/onboarding-attachments${id ? `/${encodeURIComponent(id)}` : ""}`, { method, ...(body !== undefined ? { body } : {}), signal: controller.signal });
  } catch (caught) {
