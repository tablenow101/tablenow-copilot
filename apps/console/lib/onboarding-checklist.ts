...
const providerLabels: Record<string, string> = { zenchef: "Zenchef", sevenrooms: "SevenRooms", thefork: "TheFork" };
...
export function onboardingChecklist(answers: OnboardingAnswers): SetupChecklistItem[] {
  ...
  return [
    ...
    ...providers.map(({ id, label }): SetupChecklistItem => ({ id, title: `${label} : connexion à préparer`, reason: "Outil déclaré par vous. Aucune synchronisation n’a été vérifiée ici ; vous pouvez continuer et y revenir plus tard.", complete: false, section: "reservations", step: "connections" })),
