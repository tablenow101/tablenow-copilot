...
type EmailChallenge = { stage: "email"; delivery?: "code" | "link"; expiresAt: string; expiresInSeconds: number };
...
export function AccountFlow({ mode = "login" }: { mode?: Mode }) {
  ...
  useEffect(() => {
    let live = true;
    setProgressSaved(mode === "login" && new URLSearchParams(window.location.search).get("onboarding") === "saved");
      ...
      try {
        if (mode === "verify" && window.location.hash && !started.current) {
          ...
          if ("authenticated" in next) await enterApp();
            ...
            setStage(next.stage); recordChallengeExpiry(next.expiresInSeconds);
          }
          return;
        }
        ...
        if (progress.kind === "session") router.replace(progress.onboardingComplete ? "/dashboard" : "/onboarding");
          ...
          const securityStep = progress.challenge.stage !== "email" || progress.challenge.purpose === "google" || ((progress.challenge.purpose === "login" || progress.challenge.purpose === "access") && (mode === "login" || mode === "passwordless"));
  ...
  useEffect(() => {
    if (stage === "email" && delivery === "code") codeRef.current?.focus();
  }, [stage, delivery]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (busy || needsProgressCheck || submitting.current) return;
    ...
    try {
      if (stage === "credentials") {
        ...
        const next = await api<{ authenticated: true } | EmailChallenge | ProfileChallenge>("/v1/account/verify-email", { method: "POST", body: JSON.stringify({ code }) });
  ...
  async function resend() {
    if (busy || cooldown || submitting.current) return;
    submitting.current = true;
    setBusy(true);
    setError("");
    try {
  ...
  const title = stage === "email" ? emailLink ? "Confirmez votre adresse" : "Code reçu par e-mail"
  ...
  const description = stage === "email" ? <>Nous avons envoyé {emailLink ? "un lien de confirmation" : "un code"} à <strong>{email}</strong>. {emailLink ? "Ouvrez cet e-mail pour continuer." : ""}</>
  ...
  const showSubmit = !emailLink && (mode !== "verify" || stage === "email");
  ...
  return <main className={`tn-auth tn-account theme-${theme}`} data-stage={stage}>
    <section className="tn-auth-card" aria-labelledby="auth-title" aria-busy={busy}>
      ...
      {progressSaved && stage === "credentials" && <small role="status">Votre progression a été enregistrée.</small>}
      ...
      <form key={stage} onSubmit={submit}>
        ...
        {!errorAction && (stage === "email" && !emailLink) && <>
        ...
        {errorAction === "signup" ? <Link className="tn-primary tn-account-submit" href="/register">S’inscrire</Link> : needsProgressCheck
          ...
          : restartRequired
            ...
            : showSubmit ? <button className="tn-primary tn-account-submit" type="submit" disabled={busy || (stage !== "credentials" && code.length !== 6)}>{busy && <LoaderCircle size={17} className="spinning" />}{stage !== "credentials" ? "Vérifier" : mode === "signup" ? "S’inscrire" : mode === "login" ? "Se connecter" : "Continuer"}</button> : null}
      </form>
