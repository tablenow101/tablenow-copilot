...
const publicPrivacyState: PrivacyState = {
  preferences: { productEmails: false, usageAnalytics: false, modelImprovement: false },
  requests: [],
};
...
export function PrivacyCenter() {
  ...
  const refresh = useCallback(async () => {
    if (isPublicPilotRuntime()) {
      setState(publicPrivacyState);
      return;
    }
    setState(await api<PrivacyState>("/v1/privacy"));
  }, []);
  ...
  const toggle = async (key: keyof PrivacyState["preferences"]) => {
    ...
    if (publicPilot) { setNotice("Version publique en lecture seule — aucun choix n'a été enregistré."); return; }
  ...
  const request = async (type: "export" | "deletion" | "rectification" | "restriction" | "objection", details?: string) => { if (publicPilot) { setNotice("Version publique en lecture seule — aucune demande n'a été créée."); setConfirmDeletion(false); setOtherRequest(null); setRequestDetails(""); return; } try { await api("/v1/privacy/requests", { method: "POST", body: JSON.stringify({ type, ...(details ? { details } : {}) }) }); setNotice(type === "export" ? "Votre export est en préparation." : type === "deletion" ? "Votre demande d'effacement est enregistrée avec un délai de réversibilité de 30 jours." : "Votre demande est enregistrée pour revue humaine."); setConfirmDeletion(false); setOtherRequest(null); setRequestDetails(""); await refresh(); } catch (caught) { setNotice(caught instanceof Error ? caught.message : "Impossible de créer la demande."); } };
  const cancel = async (id: string) => { if (publicPilot) { setNotice("Version publique en lecture seule — aucune demande n'a été modifiée."); return; } try { await api(`/v1/privacy/requests/${id}/cancel`, { method: "POST", body: "{}" }); setNotice("Demande annulée."); await refresh(); } catch (caught) { setNotice(caught instanceof Error ? caught.message : "Impossible d'annuler la demande."); } };
  ...
  return <AppChrome session={session} active="privacy" title="Confidentialité" subtitle="Vos choix, vos exports et vos demandes — accessibles sans passer par le support." onRefresh={() => void refresh()}>
    {publicPilot && <div className="public-pilot-banner"><ShieldCheck size={16} /><span><strong>Version pilote publique</strong> Ce centre illustre les protections TableNow sans compte, cookie de session ni donnée personnelle.</span></div>}
    <div className="privacy-layout"><section className="privacy-main"><div className="privacy-promise"><div><ShieldCheck size={24} /><span><strong>Données privées par défaut.</strong><p>La bêta n'active ni publicité, ni cookie marketing, ni réutilisation des conversations pour entraîner un modèle.</p></span></div><span>RGPD · PILOTE</span></div>{notice && <div className="notice-banner"><CheckCircle2 size={16} />{notice}</div>}
      <section className="panel privacy-section"><header><div><span className="eyebrow">Préférences facultatives</span><h2>Choisir ce qui dépasse le service essentiel</h2></div></header><Preference title="E-mails produit" text="Recevoir les nouveautés et invitations à des entretiens produit." value={state.preferences.productEmails} disabled={publicPilot} onChange={() => void toggle("productEmails")} /><Preference title="Mesure d'usage" text="Autoriser des métriques agrégées pour améliorer les parcours, sans publicité." value={state.preferences.usageAnalytics} disabled={publicPilot} onChange={() => void toggle("usageAnalytics")} /><Preference title="Amélioration des modèles" text="Autoriser l'utilisation de conversations préalablement pseudonymisées. Désactivé par défaut." value={state.preferences.modelImprovement} disabled={publicPilot} onChange={() => void toggle("modelImprovement")} /></section>
      <section className="panel privacy-section"><header><div><span className="eyebrow">Vos droits</span><h2>Agir sans passer par le support</h2></div></header><div className="rights-grid"><button disabled={publicPilot} onClick={() => void request("export")}><FileKey2 size={20} /><span><strong>Préparer mon export</strong><small>Compte, restaurants, actions et données associées dans un JSON portable.</small></span></button><button className="danger-right" disabled={publicPilot} onClick={() => setConfirmDeletion(true)}><Trash2 size={20} /><span><strong>Demander l'effacement</strong><small>Délai de 30 jours, revue des obligations et possibilité d'annuler.</small></span></button></div><div className="other-rights"><span>Autres demandes :</span><button disabled={publicPilot} onClick={() => setOtherRequest("rectification")}>Rectification</button><button disabled={publicPilot} onClick={() => setOtherRequest("restriction")}>Limitation</button><button disabled={publicPilot} onClick={() => setOtherRequest("objection")}>Opposition</button></div></section>
...
function Preference({ title, text, value, disabled = false, onChange }: { title: string; text: string; value: boolean; disabled?: boolean; onChange: () => void }) { return <div className="preference-row"><div><strong>{title}</strong><p>{text}</p></div><button className={`switch ${value ? "on" : ""}`} role="switch" aria-label={title} aria-checked={value} disabled={disabled} onClick={onChange}><i /></button></div>; }
