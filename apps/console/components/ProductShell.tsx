...
export type ProductSection = "dashboard" | "communications" | "reservations" | "operations" | "team" | "inventory" | "performance" | "locations" | "systems" | "copilot";

const meta: Record<ProductSection, { title: string; subtitle: string }> = {
  dashboard: { title: "Aujourd'hui", subtitle: "Le service, les signaux et les décisions qui comptent maintenant." },
...
export function ProductShell({ section }: { section: ProductSection }) {
  ...
  const notifications = visibleWorkspace.decisions.filter((item) => item.status === "open").slice(0, 4).map((item) => ({ id: item.id, title: item.title, detail: item.priority === "critical" ? "Décision critique" : "Décision à traiter", href: "/dashboard" }));
  ...
  return <AppChrome session={session} active={section} title={pageMeta.title} subtitle={pageMeta.subtitle} refreshing={refreshing} onRefresh={() => void refresh()} notifications={notifications} restaurants={workspace.restaurants} activeRestaurantId={effectiveRestaurantId} onRestaurantChange={selectRestaurant}>
    ...
    {section === "dashboard" && <TodayView workspace={visibleWorkspace} mutate={mutate} onNewReservation={readOnly ? requestWriteAccess : () => setReservationOpen(true)} />}
...
function TodayView({ workspace, mutate, onNewReservation }: { workspace: Workspace; mutate: Mutation; onNewReservation: () => void }) {
  ...
  const firstResult = workspace.firstResults[0];
  return <div className="view-stack">
    {firstResult && <section className="first-result-card">
      <div>
        <span className="brief-kicker"><Sparkles size={13} /> Préparé à partir de vos réponses</span>
        <h2>{firstResult.title}</h2>
        <p>Premier livrable pour {restaurantName(workspace, firstResult.restaurantId)}.</p>
      </div>
      <div className="first-result-body">
        <article><strong>Situation retenue</strong>{firstResult.confirmedFacts.map((fact) => <small key={fact}>{fact}</small>)}</article>
        <article><strong>Recommandation</strong>{firstResult.recommendations.map((item) => <small key={item}>{item}</small>)}</article>
        <article><strong>Informations manquantes</strong>{firstResult.unknownFields.length ? firstResult.unknownFields.map((item) => <small key={item}>{item}</small>) : <small>Aucune information bloquante pour ce brouillon.</small>}</article>
      </div>
      <footer><span>Statut : {firstResult.status === "ready_for_review" ? "à vérifier" : firstResult.status === "needs_information" ? "à compléter" : "brouillon"}</span><Link className="primary-button compact" href={`/onboarding?restaurantId=${encodeURIComponent(firstResult.restaurantId)}&section=operations`}>{firstResult.kind === "supplier_order" ? "Ouvrir le brouillon" : firstResult.kind === "customer_communication" || firstResult.kind === "customer_loyalty" ? "Vérifier le protocole" : firstResult.kind === "reservations" ? "Vérifier les règles" : "Ouvrir mon plan"} <ArrowRight size={14} /></Link></footer>
    </section>}
    <section className="command-brief">
      <div><span className="brief-kicker"><CircleDot size={13} /> Lecture du service</span><h2>{hasReservations ? <>{summary.coversToday} couverts sont renseignés aujourd’hui. {summary.openDecisions ? `${summary.openDecisions} décisions demandent votre attention.` : "Aucun blocage n'attend votre validation."}</> : <>Le cockpit est prêt, mais <em>aucune réservation</em> n’est encore renseignée.</>}</h2><p>{hasReservations ? "Lecture fondée sur les réservations actuellement visibles." : "Ajoutez la première demande ou connectez la méthode déjà utilisée."} {hasInventory ? (summary.inventoryAlerts ? `${summary.inventoryAlerts} niveau de stock est à surveiller avant la mise en place.` : "Les articles renseignés sont au-dessus de leur seuil.") : "Aucun état de stock n’est encore connu."}</p></div>
    ...
    <section className="kpi-grid">
      <Kpi icon={<Gauge />} label="Couverts aujourd’hui" value={hasReservations ? String(summary.coversToday) : "Non mesuré"} detail={hasReservations ? "dans les réservations visibles" : "aucune source de réservation"} tone="lime" />
      <Kpi icon={<Euro />} label="Revenu capté" value={hasMetrics ? euro(summary.revenueCapturedToday) : "Non mesuré"} detail={hasMetrics ? "attribué à TableNow aujourd'hui" : "aucune source de mesure"} />
      <Kpi icon={<Clock3 />} label="Temps rendu" value={hasMetrics ? `${Math.floor(summary.timeSavedMinutes / 60)} h ${summary.timeSavedMinutes % 60} min` : "Non mesuré"} detail={hasMetrics ? "demandes gérées automatiquement" : "aucune source de mesure"} />
      <Kpi icon={<Utensils />} label="Réservations visibles" value={hasReservations ? String(workspace.reservations.length) : "Non mesuré"} detail={hasReservations ? "dans les sources chargées" : "aucune source de réservation"} />
    </section>
...
function PerformanceView({ workspace }: { workspace: Workspace }) {
  ...
  return <div className="view-stack"><section className="impact-hero"><span className="eyebrow">Impact des 14 derniers jours</span><div><strong>{metrics.length ? euro(totalRevenue) : "Non mesuré"}</strong><p>{metrics.length ? "de revenu lié aux demandes captées et converties par TableNow." : "Aucune mesure n’est encore connectée ; TableNow n’invente pas d’impact."}</p></div></section><section className="kpi-grid three"><Kpi icon={<Clock3 />} label="Temps économisé" value={metrics.length ? `${Math.round(totalMinutes / 60)} h` : "Non mesuré"} detail={metrics.length ? "rendu aux équipes" : "aucune source de mesure"} /><Kpi icon={<TrendingUp />} label="Conversion moyenne" value={metrics.length ? `${avgConversion.toFixed(1)} %` : "Non mesuré"} detail={metrics.length ? "demande vers réservation" : "aucune source de mesure"} /><Kpi icon={<Phone />} label="Demandes traitées" value={metrics.length ? String(metrics.reduce((sum, item) => sum + item.callsHandled, 0)) : "Non mesuré"} detail={metrics.length ? "sur la période" : "aucune source de mesure"} /></section><section className="panel chart-panel"><PanelTitle eyebrow="Revenu capté" title="Chaque jour, pas une moyenne abstraite" />{metrics.length ? <div className="bar-chart" role="img" aria-label="Revenu capté sur les quatorze derniers jours">{metrics.map((metric) => <div key={metric.date} title={`${formatRestaurantDate(metric.date, "UTC")} — ${euro(metric.revenueCaptured)}`}><span style={{ height: `${Math.max(8, metric.revenueCaptured / maxRevenue * 100)}%` }} /><small>{new Date(`${metric.date}T12:00:00.000Z`).toLocaleDateString("fr-FR", { weekday: "narrow", timeZone: "UTC" })}</small></div>)}</div> : <Empty icon={<TrendingUp />} title="Aucune série mesurée" text="Les indicateurs apparaîtront après saisie ou connexion d’une source vérifiée." />}</section></div>;
}
