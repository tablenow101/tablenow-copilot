...
export function TodayOverview({ workspace, timeZone, phase, busy, onPrepare, onAddTask, onShowPreparation }: {
  ...
  return <div className="tn-today-overview">
    <section className="tn-today-hero" aria-label="Réservations enregistrées aujourd’hui"
      ...
      <div className="tn-today-metric"><strong>{phase === "after" ? data.completedCovers : data.covers}</strong><div><span>{phase === "after" ? "couverts terminés" : "couverts enregistrés"}</span><small>{data.reservations} réservation{data.reservations === 1 ? "" : "s"} du jour</small></div></div>
      ...
      </details> : <p>Aucune réservation enregistrée pour aujourd’hui. <Link href="/service">Ajouter une réservation <ArrowRight size={14} /></Link></p>}
    ...
    <div className="tn-today-next">
      <section className="tn-today-priority">
        ...
        <h2>{decision?.title || (phase === "after" ? "Préparons le prochain service." : "Une équipe prête, dès l’ouverture.")}</h2>
