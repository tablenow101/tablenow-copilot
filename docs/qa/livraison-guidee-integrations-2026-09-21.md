...
## Confirmation finale du 22 septembre

Correctif conversation : `6a114c8c1184a66882b823e272f886dd35d8be3b`, déploiement `dpl_Dk6fUVioq52uqgWUg7ueddmmrR3c`, URL `tablenow-copilot-v2-p04s0ew5p-tablenow101.vercel.app`, Preview READY et alias vérifié. Migration 015 appliquée le 21 septembre à 22:13:59 UTC. Deux runs et leurs quatre messages associés classés ensemble après contrôle des IDs, dates, tenant, restaurant et propriétaire. Deux runs métier/quatre messages métier restent accessibles. Les quatre runs et huit messages restent physiquement présents.

Après rechargement du dernier build, le parcours Accueil→Ma configuration→Zenchef→Connexions a été rejoué : historique QA absent, textes utilisateur « Ça ne fonctionne pas » et « Test » présents. Accueil sans tâche ni progression QA. Zéro erreur console collectée pendant cette recette ciblée. Petit viewport 320×568 : dernier bloc 423,1px, barre 463px, aucun débordement horizontal.

Conservation vérifiée : quatre utilisateurs, un enrôlement de compte, un brouillon, quatre runs et huit messages ; quatre traces d’audit du classement. Tâche et fichier chiffré toujours présents. Main distante reste `665ca205111c7937b1e7507137e0af2b3c161c54`. Aucun domaine ni configuration production modifié. Le commit documentaire qui suit n’altère pas cette source applicative.

**Verdict : VERIFIED sur les corrections ciblées d’accueil, navigation, isolation et composition aux formats testés ; validation produit globale ouverte.**

Restent bloqués ou non exécutés : accès partenaires Zenchef/SevenRooms/TheFork/OpenTable ; adaptateurs externes à implémenter et tester après accès ; fournisseur IA autorisé absent de Preview ; extraction PDF/images non implémentée ; nouveau compte Google/e-mail/TOTP complet, récupération réelle, passkeys, micro et clavier sur téléphone physique non validés dans cette livraison. Le catalogue de procédures n’est pas une connexion en un clic. Aucun lot final ni passage main autorisé.
