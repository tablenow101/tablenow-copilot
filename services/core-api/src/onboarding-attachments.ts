...
import { readPrivateAttachment, maxExtractedCharacters } from "./attachment-content.js";
...
export async function registerOnboardingAttachments(app: FastifyInstance, database: Database) {
  ...
  app.get("/v1/onboarding-attachments", { preHandler: guard }, async request => withTenant(database, request.actor!.tenantId, async tx => ({ extraction: { formats: ["text/plain"], maxCharacters: maxExtractedCharacters }, files: await tx`select id,name,mime_type as "mimeType",byte_size as "byteSize" from onboarding_attachments where tenant_id=${request.actor!.tenantId} and data_origin='business' order by created_at` })));
  app.post("/v1/onboarding-attachments", { preHandler: guard, bodyLimit: 2800000, config: { rateLimit: { max: 20, timeWindow: "15 minutes" } } }, async (request, reply) => {
    ...
    const result = await withTenant(database, actor.tenantId, async tx => {
      ...
      const [file] = await tx`insert into onboarding_attachments(tenant_id,user_id,name,mime_type,byte_size,encrypted_content) values (${actor.tenantId},${actor.userId!},${input.name},${input.mimeType},${bytes.length},${seal(input.base64,key)}) returning id,name,mime_type as "mimeType",byte_size as "byteSize"`;
      return file;
    });
  ...
  app.get("/v1/onboarding-attachments/:id/extraction", { preHandler: guard }, async (request, reply) => {
    const { id } = z.object({ id: z.uuid() }).parse(request.params);
    const file = await readPrivateAttachment(database, request.actor!, id);
    return file || reply.code(404).send({ error: { code: "COPILOT_ATTACHMENT_UNAVAILABLE", message: "Ce document n’est pas disponible pour ce compte." } });
  });
  app.get("/v1/onboarding-attachments/:id", { preHandler: guard }, async (request, reply) => {
    const { id } = z.object({ id: z.uuid() }).parse(request.params);
    const [file] = await withTenant(database, request.actor!.tenantId, tx => tx<{ name: string; encrypted_content: string }[]>`select name,encrypted_content from onboarding_attachments where id=${id} and tenant_id=${request.actor!.tenantId} and data_origin='business'`);
  ...
  app.delete("/v1/onboarding-attachments/:id", { preHandler: guard }, async (request, reply) => {
    const { id } = z.object({ id: z.uuid() }).parse(request.params);
    await withTenant(database, request.actor!.tenantId, tx => tx`delete from onboarding_attachments where id=${id} and tenant_id=${request.actor!.tenantId} and data_origin='business'`);
