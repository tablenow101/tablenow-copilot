...
import { exchangeGoogleCode, googleAuthorizationUrl, googleConfiguration, type GoogleExchange, type GoogleIdentity } from "./google-identity.js";

type Attempt = { verifier: string; nonce: string; rememberMe: boolean };
...
export async function registerGoogleRoutes(app: FastifyInstance, database: Database, beginAccount: BeginAccount, exchange: GoogleExchange = exchangeGoogleCode) {
  ...
  app.get("/v1/oauth/google/start", rate, async (request, reply) => {
    ...
    await database`insert into google_login_attempts(state_hash,browser_hash,payload) values (${digest(state)},${digest(browser)},${seal({ verifier, nonce, rememberMe: input.remember === "1" }, secret)})`;
  ...
  app.get("/v1/oauth/google/callback", rate, async (request, reply) => {
    ...
    try {
      const host = String(request.headers["x-forwarded-host"] || request.headers.host).split(",")[0]!.trim();
      if (host !== new URL(config.origin).host) throw new Error();
      ...
      const identity = await exchange(config, input.code, attempt.verifier, attempt.nonce);
      await beginAccount(identity, attempt.rememberMe, reply);
      return reply.redirect(`${config.origin}/login?google=continue`);
    } catch {
