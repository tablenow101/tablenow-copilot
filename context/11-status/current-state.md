# Google OAuth et domaine historique — correction prête, 16 septembre 2026

La Preview canonique sert le commit `d8f2ec8a0d94d4ca932c1a8ad4d6a6ba56f772ea` sur `preview.tablenow.io`. Le bouton Google a réellement renvoyé dans TableNow jusqu'à l'écran TOTP avec le client final ; Google Places et l'e-mail sont annoncés prêts par la vérification serveur. Le nouvel échec Chrome provient de `copilot.tablenow.io`, encore affecté à une ancienne Preview. Le correctif redirige cet hôte vers `preview.tablenow.io` en Preview et vers `os.tablenow.io` en production. Validation locale : lint 16/16, typage 16/16, 209 tests réussis, 4 intégrations ignorées, build 11/11. Publication et test du nouvel alias restent à exécuter ; TOTP complet, e-mail reçu, onboarding, cockpit et appareils mobiles ne sont pas encore certifiés.
