...
Le propriétaire limite désormais le travail au seul bloc authentification. La cible validée est une entrée unique : logo officiel dans la carte, Google puis e-mail, code à six chiffres, session immédiate pour un compte existant et demande du nom seulement après vérification d’une nouvelle adresse. Aucun mot de passe, TOTP, QR ou code de secours n’est exposé. Décision canonique 102 relue. Commit `265d7595f4855324803f8413202fd157aff2492b`, déploiement `dpl_CsiCiQfVxQof6Hw87eyqnP6Qd5Va` READY et alias `preview.tablenow.io` confirmés. `/login` et `/register` servent la même entrée ; logo dans la carte, ordre Google puis e-mail, absence d’ancien parcours et absence de débordement vérifiés dans le navigateur déployé à 1363 × 936. 142 tests API et 131 tests console passent ; lint, types et build réussissent. `main`, la production, l’onboarding et le cockpit demeurent inchangés. La réception réelle du code, Google avec un compte réel et les appareils physiques restent `NOT_RUN` : aucun e-mail ni compte réel n’a été déclenché pendant la recette publique.
...
La demande actuelle autorise les corrections TOTP sur un téléphone, guidage, états vides et pièces jointes en Preview ; mémoire canonique 99 version 7, enregistrée puis relue. Main et production restent hors périmètre. Les 60 fichiers suivis absents sont restaurés après contrôle : aucun checkout partiel ou opération Git active, sauvegarde des modifications, puis restauration limitée aux absents depuis HEAD vérifié. SHA-256 inchangés pour les 316 fichiers suivis déjà présents. Les tests API de reprise/authentification (17) et de persistance onboarding sur trois profils (3) passent ; les vérifications de rendu et la recette Preview des nouvelles corrections restent en cours. Aucun lot clôturé. [Diagnostic ciblé et recette à accomplir](../../docs/qa/parcours-guide-2026-09-21.md).
